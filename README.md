...
ファイル共有サーバ上のtessaractOCRを最新版に上書きするとこのスクリプトを実行したPCのDドライブ上のtessaractOCRも最新版に更新されます。
ファイル共有サーバ上のtessaractOCRはコピー時に読み取られるだけであり、PC上で実行中にファイル共有サーバ上のファイルがロックされる等がないことから最新バージョンへの上書き更新でいつでもPC側に最新版を配置することが可能なスクリプトとしています。
