...
tessaractOCR本体はサイズが大きいためファイル共有サーバ上に配置する仕様とし、スクリプトを実行したPCのDドライブの所定のフォルダに配置されることを想定しています。　PCのDドライブにtessaractOCRがなければファイル共有サーバからフォルダがコピーされます。　したがって、ファイル共有サーバ上のtessaractOCRを最新版に上書きすると、このスクリプトを実行したPCのDドライブ上のtessaractOCRも最新版に更新されます。　ファイル共有サーバ上のtessaractOCRはコピーの際に読み取られるだけであるため、ロックされることがないので最新バージョンへの上書き更新が随時可能です。
